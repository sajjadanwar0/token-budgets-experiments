#!/usr/bin/env python3
"""
b0_sweep.py — Forgetful-Operator parameter sweep.

Purpose
-------
Reviewers 2 and 3 argued the Forgetful-Operator overshoot is a *staged
artifact* of B0 = 60 (3 children x ~23 uc = 69 > 60). This harness sweeps the
budget across the boundary so the paper can show the racy pattern is
parameter-dependent in the EXPECTED direction (overshoot when 3*per_child > B0,
safe otherwise), while the affine-split discipline is cap-respecting at EVERY
B0 -- i.e. the integrity property does not depend on the operator picking a
"lucky" cap.

Four conditions, matching paper section 5.11:
  A  racy asyncio (shared mutable counter, no lock)   -- expected to overshoot when 3*c > B0
  B  locked asyncio (counter under asyncio.Lock)      -- cap-respecting (operator remembered the lock)
  C  affine-split Rust-style model (Python emulation) -- cap-respecting BY CONSTRUCTION
  E  Rust shared Arc<Mutex<Budget>> (emulated)        -- cap-respecting (operator remembered the lock)

B0 grid (per the recommendation):
  50  -> 3*c=69 > 50    : racy MUST overshoot (deep)
  69  -> 3*c=69 = 69    : boundary (racy marginal)
  100 -> 3*c=69 < 100   : racy ACCIDENTALLY safe  <-- the key cell that proves parameter-dependence
plus the paper's original 60 for continuity.

LIVE MODE (real Anthropic calls) is opt-in. The default is a deterministic
offline simulation that reproduces the race arithmetic exactly, so the
parameter-dependence claim can be regenerated with no API key. Run live to
confirm the simulation matches reality.

Usage
-----
  python3 b0_sweep.py                       # offline deterministic sim (no key)
  python3 b0_sweep.py --live --trials 30    # live Anthropic (~$0.01 total)
  python3 b0_sweep.py --b0 50 69 100 --children 3 --per-child 23

Output
------
  b0_sweep_results.csv  (one row per trial)
  prints a summary table: condition x B0 -> overshoot/trials
"""
