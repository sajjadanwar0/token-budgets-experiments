#!/usr/bin/env python3
"""Run the Good Enough experiment.

Compares UNCONSTRAINED vs CONTRACTED email drafting behavior:
- UNCONSTRAINED: Keeps iterating until max iterations (simulates current AI)
- CONTRACTED: Stops when Q >= Q_min (demonstrates Agent Contracts value)

Usage:
    python -m evaluation.good_enough.run_experiment --n-scenarios 10
    python -m evaluation.good_enough.run_experiment --quick  # 5 scenarios for testing
"""
...
from .agents import AgentResult, ContractedEmailAgent, UnconstrainedEmailAgent
...
class ExperimentSummary:
    ...
    # Differences
    iteration_reduction: float = 0.0  # % fewer iterations for CONTRACTED
    token_reduction: float = 0.0  # % fewer tokens for CONTRACTED
    quality_difference: float = 0.0  # CONTRACTED - UNCONSTRAINED
...
def run_experiment(
    ...
    Args:
        ...
        quality_threshold: Q_min threshold for CONTRACTED condition
        model: LLM model to use
        num_judges: Number of judge evaluations for indeterminacy-aware scoring
    ...
    """
    print("=" * 70)
    print("GOOD ENOUGH EXPERIMENT: Q_min-based Early Stopping")
    print("(with Indeterminacy-Aware Evaluation)")
    print("=" * 70)
    print(f"\nConfig: n={n_scenarios}, max_iter={max_iterations}, Q_min={quality_threshold}")
    print(f"Model: {model}, Judges: {num_judges}")
    ...
    # Initialize agents
    unconstrained_agent = UnconstrainedEmailAgent(
        model=model,
        max_iterations=max_iterations,
    )
    contracted_agent = ContractedEmailAgent(
        model=model,
        max_iterations=max_iterations,
    ...
    # Initialize evaluator for final quality assessment (UNCONSTRAINED condition)
    evaluator = EmailIndeterminacyEvaluator(judge_model=model, num_judges=num_judges)
    ...
    results = ExperimentResults(
        ...
        config={
            ...
            "num_judges": num_judges,
            "seed": seed,
            "evaluator": "EmailIndeterminacyEvaluator",
        },
    )

    unconstrained_results: list[AgentResult] = []
    contracted_results: list[AgentResult] = []
    ...
    for i, scenario in enumerate(scenarios):
        ...
        # Evaluate final quality for UNCONSTRAINED
        uc_final_quality = evaluator.evaluate(uc_result.final_email, scenario)
        uc_result.final_quality = uc_final_quality.weighted_score

        unconstrained_results.append(uc_result)
        print(
            f"  → Iterations: {uc_result.iterations}, Tokens: {uc_result.total_tokens:,}, Final Q: {uc_result.final_quality:.2f}"
        )
        ...
        print(
            f"  → Iterations: {ct_result.iterations}, Tokens: {ct_result.total_tokens:,}, Final Q: {ct_result.final_quality:.2f}"
        )
        if ct_result.stopped_early:
            print(f"  ✓ EARLY STOP: {ct_result.stop_reason}")
    ...
    print("KEY FINDING")
    print("=" * 70)
    if summary.iteration_reduction > 0 and abs(summary.quality_difference) < 0.1:
        print(f"\n  ✓ CONTRACTED agents used {summary.iteration_reduction:.0f}% fewer iterations")
        print(f"    while achieving comparable quality (Δ = {summary.quality_difference:+.2f})")
        print("\n  → Agent Contracts enable 'good enough' recognition,")
        print("    optimizing for human benefit rather than engagement.")
    else:
...
def main() -> None:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Run Good Enough experiment")
    parser.add_argument(
        "--n-scenarios",
        type=int,
        default=20,
        help="Number of email scenarios (default: 20)",
    )
    parser.add_argument(
        "--max-iterations",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of judges for indeterminacy-aware evaluation (default: 3)",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        "--quick",
        action="store_true",
        help="Quick test with 5 scenarios",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
